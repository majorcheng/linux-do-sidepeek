...
1. 打开 `https://linux.do/latest`
2. 点击目标主题标题打开抽屉
3. 读取任一帖子卡片底部的 `.ld-post-actions`

断言：

1. 操作条从左到右依次为 `点赞 / 复制链接 / 收藏 / 回复`
2. 操作条整体靠左对齐，不再贴右侧
3. 不出现 `举报` 按钮

### AGENT-CHROME-012：只看楼主
...
建议目标：
优先选楼主至少有两条发言、且其他用户也有回复的公开主题

步骤：

1. 打开任一主题抽屉
2. 在 `选项` 中把 `作者过滤` 切到 `只看楼主`
3. 观察当前抽屉中的帖子卡片
